...
## 2. Watch Product Model Improvements

### Current model

The current `Watch` model has:
...
Expand the watch model so the frontend can display a complete product page.
...
- `shortDescription`: small text for listing cards.
- `price`: selling price.
- `compareAtPrice`: old price or crossed-out price.
- `currency`: default `LKR` or your required currency.
- `images`: array of product image URLs.
- `thumbnail`: main product image.
- `category`: example values: `men`, `women`, `kids`, `luxury`, `smart`, `classic`.
- `collection`: example values: `new-arrivals`, `best-sellers`, `premium`.
- `movementType`: example values: `quartz`, `automatic`, `digital`, `smart`.
- `caseMaterial`: example values: `stainless steel`, `alloy`, `plastic`, `ceramic`.
- `strapMaterial`: example values: `leather`, `metal`, `silicone`, `fabric`.
- `waterResistance`: example: `30m`, `50m`, `100m`.
- `color`: watch color.
- `dialColor`: dial color.
- `size`: watch case size.
- `sku`: unique stock keeping unit.
- `stockQuantity`: number of items available.
- `inStock`: true or false.
- `isFeatured`: show on homepage.
- `isPublished`: visible to customers.
- `ratingAverage`: average customer rating.
- `ratingCount`: number of reviews.

### Validation rules to add

- `name`, `brand`, `price`, `sku`, and `category` should be required.
- `price` should not be negative.
- `compareAtPrice` should be greater than or equal to `price` when provided.
- `sku` should be unique.
- `slug` should be unique.
- `stockQuantity` should not be negative.
- `images` should support multiple URLs.
