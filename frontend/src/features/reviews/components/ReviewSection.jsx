    ...
    <section className="mt-8 rounded-lg border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(28,41,56,0.06)] lg:p-7">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="grid gap-3">
          {isLoading ? (
            <LoadingState label="Loading reviews" variant="reviews" rows={3} />
          ) : reviews.length > 0 ? (
            reviews.map((review) => (
              <ReviewCard
...
const ReviewForm = ({ form, isEditing, isSubmitting, onCancel, onChange, onSubmit }) => {
  ...
  return (
    <form className="grid gap-4" onSubmit={onSubmit}>
      ...
      <div className="flex flex-wrap gap-2">
        <button className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg bg-teal-700 px-4 font-extrabold text-white hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-65" disabled={isSubmitting} type="submit">
          {isSubmitting && <ButtonSpinner />} {isSubmitting ? 'Saving' : isEditing ? 'Update review' : 'Submit review'}
        </button>
        {isEditing && (
