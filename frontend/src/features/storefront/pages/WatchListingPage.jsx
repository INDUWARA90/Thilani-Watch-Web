...
const fieldClass = 'h-11 min-w-0 w-full rounded-lg border border-primary/15 bg-card px-3.5 text-base font-normal text-primary outline-none transition duration-200 placeholder:text-primary/60 focus:border-accent focus:ring-2 focus:ring-accent/35'
...
export const WatchListingPage = () => {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-base pb-16 text-primary">
      <section className="relative overflow-hidden bg-base px-4 pb-28 pt-20 text-primary sm:px-6 sm:pt-24 lg:px-10">
        <div className="relative z-10 mx-auto flex max-w-[1200px] min-w-0 flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl min-w-0">
            <p className="mb-4 inline-flex min-h-9 items-center rounded-full border border-primary/15 bg-card px-4 text-xs font-semibold uppercase text-primary/70 shadow-premiumSm">
              Storefront
            </p>
            <h1 className="break-words font-heading text-4xl font-bold leading-[1.05] text-primary sm:text-[56px] lg:text-[72px]">
              Watches Collection
            </h1>
            <p className="mt-4 text-sm font-normal leading-relaxed text-primary/75 sm:text-base lg:text-lg">
          ...
          <motion.div whileHover={{ scale: 1.03 }}>
            <Link className="inline-flex min-h-11 w-fit max-w-full shrink-0 items-center justify-center rounded-full border border-primary/20 bg-card px-5 text-sm font-bold text-primary no-underline shadow-premiumSm transition duration-200 hover:border-accent hover:text-accent active:scale-98 focus:outline-none focus:ring-2 focus:ring-accent sm:mb-2 sm:px-8" to="/">
              Back home
            </Link>
          </motion.div>
        </div>
      ...
      <section className="mx-auto grid max-w-[1200px] min-w-0 gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[290px_minmax(0,1fr)] lg:px-10">
          ...
          <div className="flex flex-col gap-4">
            <label className="grid gap-1.5 text-sm font-medium text-primary">
              ...
              <span className="flex h-11 items-center rounded-lg border border-primary/15 bg-base px-3.5 transition duration-200 focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/35">
                ...
                <input className="min-w-0 flex-1 bg-transparent px-2.5 text-base font-normal text-primary outline-none placeholder:text-primary/60" placeholder="Search watches..." value={searchValue} onChange={(event) => setSearchValue(event.target.value)} />
              </span>
            </label>
        ...
        <section className="min-w-0 overflow-hidden">
          <div className="mb-6 flex flex-col gap-4 rounded-lg border border-primary/10 bg-card p-5 shadow-premiumSm sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase text-primary/75">{pagination.total || watches.length} watches found</p>
              <h2 className="mt-0.5 font-heading text-xl font-bold text-primary">Collection Results</h2>
            </div>
            <label className="flex w-full min-w-0 flex-wrap items-center gap-3 text-sm font-medium text-primary sm:w-auto sm:flex-nowrap">
              <span className="flex items-center gap-1.5 whitespace-nowrap text-primary"><SlidersHorizontal className="h-4 w-4 text-accent" /> Sort by</span>
          ...
          {isLoading ? (
            ...
            <>
              ...
              {watches.length === 0 && (
                <div className="rounded-lg border border-dashed border-primary/15 bg-card px-4 py-16 text-center font-medium text-primary/75 shadow-premiumSm">
...
const Pagination = ({ pagination, updateFilter }) => (
  <div className="mt-8 flex flex-col gap-4 rounded-lg border border-primary/10 bg-card p-4 shadow-premiumSm sm:flex-row sm:items-center sm:justify-between">
    <p className="min-w-0 break-words pl-2 text-sm font-medium text-primary/75 [&_span]:!text-primary">
      Page <span className="font-semibold text-primary">{pagination.page}</span> of <span className="font-semibold text-primary">{pagination.pages}</span> - <span className="text-primary/75">{pagination.total} timepieces</span>
    </p>
    <div className="grid grid-cols-2 gap-3 sm:flex">
      <button className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-full border border-primary/15 bg-card px-4 text-sm font-bold text-primary transition duration-200 hover:border-accent hover:text-accent hover:shadow-premiumSm disabled:cursor-not-allowed disabled:opacity-50 active:scale-98 focus:outline-none focus:ring-2 focus:ring-accent sm:px-6" disabled={!pagination.hasPrevPage} type="button" onClick={() => updateFilter('page', String(Math.max(1, pagination.page - 1)))}>
        Previous
      </button>
      <button className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-full bg-accent px-4 text-sm font-bold text-primary transition duration-200 hover:bg-accent/90 hover:shadow-premium disabled:cursor-not-allowed disabled:opacity-50 active:scale-98 focus:outline-none focus:ring-2 focus:ring-primary sm:px-6" disabled={!pagination.hasNextPage} type="button" onClick={() => updateFilter('page', String(pagination.page + 1))}>
        Next
      </button>
    </div>
  </div>
