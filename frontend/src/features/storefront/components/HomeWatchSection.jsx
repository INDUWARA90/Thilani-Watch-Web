...
export const HomeWatchSection = ({ eyebrow, title, text, watches, isLoading }) => (
 ...
 <p className="inline-flex min-h-8 items-center border-b border-accent/60 px-0 pb-2 text-xs font-bold uppercase tracking-[0.24em] text-black">{eyebrow}</p>
        <div className="mt-5 h-px w-20 bg-accent" aria-hidden="true" />
        <h2 className="mt-5 font-heading text-4xl font-bold leading-none tracking-wide text-black sm:text-6xl">{title}</h2>
        <p className="mt-3 text-sm leading-7 text-black">{text}</p>
      </div>
      <motion.div whileHover={{ scale: 1.03 }}>
        <Link
          className="inline-flex min-h-11 w-fit items-center justify-center gap-2 rounded-full border border-accent/40 bg-card px-6 text-sm font-bold text-black no-underline shadow-premiumSm transition duration-200 hover:-translate-y-0.5 hover:border-accent hover:text-accent hover:shadow-goldHairline focus:outline-none focus:ring-2 focus:ring-accent"
          to="/watches"
        >
    ...
    {isLoading ? (
      ...
      <div className="rounded-lg border border-dashed border-primary/15 bg-card px-5 py-12 text-center shadow-premiumSm">
        <p className="font-heading text-lg font-bold text-black">No watches here yet</p>
        <p className="mt-2 text-sm text-black">This section will fill automatically when matching watches are added.</p>
      </div>
    )}
  </motion.section>
)
