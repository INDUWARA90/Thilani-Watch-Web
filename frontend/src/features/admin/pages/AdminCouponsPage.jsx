    ...
    <div className="w-full flex flex-col text-sm max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1 text-xs text-primary/60">Configure and manage loyalty coupon rules and seasonal promotional codes</p>
        </div>
        <button
          type="button"
          onClick={openForm}
              ...
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200/80">
                  {['Code Key', 'Redemption Rule', 'Min Spend', 'Redemption Depth', 'Expiration Timeline', 'Status', 'Configuration'].map((heading) => (
                    <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-primary/60/90" key={heading}>{heading}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={getId(coupon)} className="hover:bg-slate-50/60 transition-colors group">
                      <td className="px-5 py-4 font-bold text-primary tracking-wide">
                        <span className="font-sans bg-slate-100 text-primary px-2 py-1 rounded text-xs border border-slate-200/40">
                          {coupon.code}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-4 text-primary/75 font-medium">{coupon.minimumOrderAmount ? formatMoney(coupon.minimumOrderAmount) : <span className="text-primary/45 italic font-normal">No Minimum</span>}</td>
                      <td className="px-5 py-4 text-primary/75 text-xs font-medium">
                        Limit <span className="text-primary font-semibold">{coupon.perUserLimit ?? '1'}</span> / account
                      </td>
                      <td className="px-5 py-4 text-primary/75 font-medium">{formatDate(coupon.expiresAt)}</td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold border transition-colors ${
                          isActive 
                            ...
                            : 'bg-slate-50 text-primary/45 border-slate-200'
                        }`}>
                  ...
                  <tr>
                    <td className="p-12 text-center" colSpan={7}>
                      <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                        ...
                        <p className="text-xs text-primary/45 mt-1 mb-4">There are no discount coupon configurations configured yet. Get started by creating your first promotional code.</p>
                        <button
                          type="button"
                          onClick={openForm}
                          className="inline-flex h-8 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-primary hover:bg-slate-50 cursor-pointer shadow-sm"
                        >
                          Add New Coupon
                        </button>
                      </div>
          ...
          <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white border border-slate-200/80 shadow-[0_24px_50px_-12px_rgba(15,23,42,0.25)] animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-slate-50/50">
              ...
              <button 
                ...
                disabled={isSaving}
                className="grid h-8 w-8 cursor-pointer place-items-center rounded-lg border border-slate-200 bg-white text-primary/60 hover:text-primary hover:bg-slate-100 transition-colors disabled:cursor-not-allowed disabled:opacity-60"
              ...
              <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-primary/75">Discount Allocation</label>
...
const Field = ({ label, onChange, required = false, type = 'text', value, placeholder }) => (
  <div className="flex flex-col gap-1.5">
    <label className="text-xs font-semibold text-primary/75">
...
const inputClass = 'w-full min-w-0 rounded-xl border border-slate-200/80 bg-white px-3.5 py-2.5 text-primary placeholder-slate-400 outline-none text-sm transition-all focus:border-teal-600 focus:ring-4 focus:ring-teal-600/5'
...
const secondaryBtnClass = `${baseBtnClass} border border-slate-200 bg-white text-primary hover:bg-slate-50 hover:text-primary hover:border-slate-300 shadow-sm`
...
const smallButtonClass = 'inline-flex h-8 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-bold text-primary hover:bg-slate-50 hover:text-primary transition-colors cursor-pointer shadow-sm'
